...
def analyze_building_satellite(image_bytes: bytes) -> dict[str, Any] | None:
    ...
    try:
        ...
        err = str(e)
        el = err.lower()
        if "429" in err or "RESOURCE_EXHAUSTED" in err or ("quota" in el and "exceed" in el):
            logger.warning("Gemini roof analysis hit quota/rate limit; falling back to catalog/mock.")
        elif "unavailable" in el or "high demand" in el or ("503" in err and "gemini" in el):
            logger.warning("Gemini roof analysis: model temporarily overloaded; falling back to catalog/mock.")
        else:
