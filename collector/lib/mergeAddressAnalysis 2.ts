...
/**
 * Merge GET /analyze-building output with a catalog row from the same state so downstream
 * UI (water economics) can reuse state rainfall and reference $/1k gal without new APIs.
 */
export function mergeAnalyzeIntoBuildingEnriched(
  ...
  return {
    ...
    data_notes:
      "Address flow via GET /analyze-building (Geocoding + Static Maps + Gemini). State rainfall and $/1k gal from catalog context row.",
