...
|--------|-----------------|----------------|
| [`backend/scripts/ingest_ms_buildings.py`](../backend/scripts/ingest_ms_buildings.py) | [Microsoft US Building Footprints](https://github.com/microsoft/USBuildingFootprints) GeoJSON (ODbL) from Microsoft’s published CDN zips | **`--all-states`** ingests all 50 states + DC (prototype: use `--min-sqft 100000` and `--max-per-state` 200–500). Output often `data/buildings_microsoft_nationwide.csv`. **Large downloads**; zips cached under `data/cache/ms_footprints/`. **ijson** streaming. |
| [`backend/scripts/merge_buildings_catalog.py`](../backend/scripts/merge_buildings_catalog.py) | (local merge) | Combines Microsoft CSV + `buildings.csv`; **drops** synthetic rows for any state present in the Microsoft file (unless `--no-auto-exclude-synthetic`). |
| [`backend/scripts/verify_buildings_catalog_coverage.py`](../backend/scripts/verify_buildings_catalog_coverage.py) | (local report) | Prints per-state Microsoft vs synthetic counts and `state_context` gaps. |
...
```bash
cd backend
# Nationwide Microsoft (long-running; omit --all-states and pass --states for a subset):
...
|--------|--------------------------|----------------------|
| Building footprints | **Merged** `buildings_catalog.csv` (Microsoft TX sample + synthetic AZ/PA). Legacy `buildings.csv` is synthetic-only input to `merge_buildings_catalog.py`. | Microsoft CDN → `ingest_ms_buildings.py` → e.g. `buildings_microsoft_tx_sample.csv` or multi-state `buildings_microsoft.csv`; then **`merge_buildings_catalog.py`** to refresh the catalog. |
...
| `backend/data/buildings.csv` | `buildings` (via merge) | Synthetic **non–Microsoft-only** seed: `generate_building_seed_csv.py` output; merged into `buildings_catalog.csv` for states without Microsoft rows (e.g. AZ, PA). |
| `backend/data/buildings_microsoft_tx_sample.csv` | `buildings` (via merge) | Texas subset from **Microsoft US Building Footprints** (ODbL; `ingest_ms_buildings.py`). `data_source=microsoft_us_building_footprints`, `roof_area_sqft` from polygon area. |
| `backend/data/buildings_catalog.csv` | `buildings` | **Default** seed input: Microsoft TX rows + synthetic rows for other states. **API/CSV fallback** (`database/db.py`) prefers this file over `buildings.csv` when present. |
| `backend/data/buildings_microsoft.csv` (after full ingest) | `buildings` | Multi-state Microsoft output; pass to **`merge_buildings_catalog.py --microsoft …`** to rebuild `buildings_catalog.csv`. |
