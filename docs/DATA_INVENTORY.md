...
When `DATABASE_URL` is set, Alembic migrations create tables; **`seed_database.py`** / **`populate_database.py`** load CSVs and compute derived rows. Without Postgres, **`database/db.py`** reads **`backend/data/state_context.csv`** and **`backend/data/buildings_catalog.csv`** when present (else **`backend/data/buildings.csv`**).
...
| **`buildings`** | Site id, name, state, city, `roof_area_sqft`, lat/lon, optional `footprint_geom`, `data_source` | **Imported** from **`buildings_catalog.csv`** (Microsoft TX + synthetic AZ/PA) or legacy `buildings.csv`; see [`merge_buildings_catalog.py`](../backend/scripts/merge_buildings_catalog.py) and [`ingest_ms_buildings.py`](../backend/scripts/ingest_ms_buildings.py). **No live API** during normal HTTP requests. |
